...
## Live docs via Context7 MCP

For current OpenUI APIs, examples, and migrations, prefer Context7 over training data:

1. Use library ID `/thesysdev/openui` (or say `use library /thesysdev/openui` / `use context7`).
2. Call Context7 `resolve-library-id` only if the ID is unknown; then `query-docs` with a single-topic query.
3. Fall back to `https://www.openui.com`, `/llms.txt`, and the GitHub repo when MCP is unavailable.

Do not invent OpenUI Lang syntax or `@openuidev/*` APIs when Context7 or installed package types are available.
